...
│ - 遍历 Fiber 树，执行 Diff 算法                                  │
│ - 标记需要更新的 Fiber 节点（打 flags 标记）                       │
│ - ✅ 可中断、可暂停、可恢复                                      │
│ - 使用 Scheduler 调度器管理优先级                                │
└──────────────────────────────────────────────────────────────┘
                              ↓
┌──────────────────────────────────────────────────────────────┐
│ 阶段 2: Commit（提交阶段）                                      │
│ - 根据 flags 执行实际的 DOM 操作（增删改）                         │
│ - 执行生命周期钩子（componentDidMount 等）                      │
│ - ❌ 不可中断（必须一次性完成）                                  │
└──────────────────────────────────────────────────────────────┘
```
...
│     - 执行 render 方法，生成虚拟 DOM                              │
│                                                                 │
│     ↓                                                           │
│                                                                 │
│  2. Commit 阶段 - Phase 1: Before Mutation（DOM 操作前）          │
│     - getSnapshotBeforeUpdate                                   │
│     - useLayoutEffect 清理函数（cleanup）                        │
│                                                                 │
│     ↓                                                           │
│                                                                 │
│  3. Commit 阶段 - Phase 2: Mutation（DOM 操作）                   │
│     - 执行 DOM 增删改操作                                        │
│     - 更新 DOM 节点属性                                          │
│                                                                 │
│     ↓                                                           │
│                                                                 │
│  4. Commit 阶段 - Phase 3: Layout（布局阶段）★★★                 │
│     - useLayoutEffect 执行 ← 在这里同步执行                       │
│     - 可读取最新 DOM 布局信息                                      │
│     - 可同步触发重渲染（阻塞浏览器绘制）                           │
│                                                                 │
│     ↓                                                           │
...
│     ↓                                                           │
│                                                                 │
│  6. Commit 阶段 - Phase 4: Passive（被动阶段）★★★                │
│     - useEffect 执行 ← 在这里异步执行                            │
│     - 不阻塞浏览器绘制                                           │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
```
传统渲染：10000 项 → 创建 10000 个 DOM 节点 → 卡顿
...
│  3. 返回完整的 HTML 文档                                         │
│     - 包含初始内容的 HTML                                        │
│     - 嵌入应用状态（__INITIAL_STATE__）                          │
│     - 加载 JavaScript 脚本                                       │
│     ↓                                                           │
│  4. 客户端接收并显示内容（无需等待 JS 加载完成）                    │
│     ↓                                                           │
│  5. JavaScript 加载完成                                         │
│     ↓                                                           │
│  6. Hydration（注水）                                           │
