...
> **v2.0.0 更新**：整合阶段采用评分优先 + 决策树方案，解决多草稿重叠和内容冲突问题。
...
为每个草稿生成量化评分，详见 `references/post-processing-pipeline.md` §0。
...
| 标题相同 | 保留高分草稿标题 |
| 概念重复（相同定义） | 保留高分草稿定义 |
| 概念重复（冲突定义） | 应用来源优先级规则（P0 > P1 > P2 > P3） |
| 代码重复（互补） | 合并为完整示例集 |
| 代码重复（相同） | 保留高分草稿示例 |
| 来源重复 | 合并到统一引用列表 |
...
**详见：** `references/post-processing-pipeline.md` §1.2-§1.3

### 5.4 整合操作步骤

```
Step 1: 加载评分报告（.work/[主题]/drafts/score-report.md）
Step 2: 检测重叠（标题、概念、代码、来源）
Step 3: 对每组重叠应用决策树
Step 4: 执行合并操作：
        - 保留高分 → 删除低分冗余内容
        - 合并互补 → 拼接多个版本
        - 标注冲突 → 格式化多观点呈现
Step 5: 统一术语（生成术语表，执行 sed 替换）
Step 6: 统一格式（代码块语言标注、表格分隔线）
Step 7: 输出整合后文档 + 整合报告
```
...
*参考文档版本：2.0.0 | research Skill v14.0.0+*
*更新：2026-04-27 整合阶段新增评分机制 + 合并决策树 + 来源优先级规则*
