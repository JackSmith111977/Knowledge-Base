...
**Agent 生成问题时应优先选择以下角度（与变式引擎 6 维度角度对齐）：**

| 角度类型 | 角度 ID | 适用场景 | 示例模板 |
|----------|---------|----------|----------|
| **对比型** | `contrast` | 有相似概念时 | 「XXX 和 YYY 的核心区别是什么？为什么需要两种方案？」 |
| **演进型** | `analysis` | 有历史背景时 | 「在 XXX 出现之前，开发者是怎么解决这个问题的？」 |
| **权衡型** | `analysis` | 有取舍决策时 | 「这个方案的优势是 X，那它付出了什么代价？」 |
| **边界型** | `scenario` | 有条件限制时 | 「在什么情况下这个方案会失效？」 |
| **深层型** | `analysis` | 有原理机制时 | 「为什么官方要这样设计？背后的核心思想是什么？」 |
| **应用型** | `application` | 有实践场景时 | 「你现在的项目中哪里可以用到这个？」 |
