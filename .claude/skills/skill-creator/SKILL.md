---
name: skill-creator
description: 通过问答引导创建 Skill，包含 9 种类型模板、检查清单、支持读取修改已有 Skill、Pipeline 进度追踪、AskUserQuestion 反转询问
...
7. **AskUserQuestion 优先**：所有反转模式询问用户时，优先使用 `AskUserQuestion` 工具，而非自由文本提问
...
**反转询问：** 需要用户确认/选择时，使用 `AskUserQuestion` 工具（单选/多选、header、preview）。
...
*Skill 版本：10.1.0 | 作者：Kei | 创建日期：2026-03-25*
*更新：2026-04-27 v10 → v10.1 反转询问优先使用 AskUserQuestion 工具*
