...
```
计算过程（CSS 规范公式：每项收缩量 = 溢出量 × (shrink × basis) / Σ(shrink × basis)）：
1. 空间不足 = 1200px - 1000px = 200px（需要收缩）
2. Σ(shrink × basis) = 1×300 + 1×500 + 1×400 = 1200
3. 每项收缩量 = 溢出量 × (shrink × basis) / 1200：
   - .item1 收缩 = 200 × 300/1200 = 50px
   - .item2 收缩 = 200 × 500/1200 ≈ 83.33px
   - .item3 收缩 = 200 × 400/1200 ≈ 66.67px
4. 最终尺寸：
   - .item1 = 300px - 50px = 250px
   - .item2 = 500px - 83.33px ≈ 416.67px
   - .item3 = 400px - 66.67px ≈ 333.33px

注意：收缩量不是均分的！basis 越大的项目收缩量越多（加权比例分配）。
```
...
2. Σ(shrink × basis) = 1×300 + 2×500 + 1×400 = 1700
3. 每项收缩量 = 溢出量 × (shrink × basis) / 1700：
   - .item1 收缩 = 200 × 300/1700 ≈ 35.29px
   - .item2 收缩 = 200 × 1000/1700 ≈ 117.65px（shrink=2 × basis=500，权重最大）
   - .item3 收缩 = 200 × 400/1700 ≈ 47.06px
4. 最终尺寸：
   - .item1 = 300px - 35.29px ≈ 264.71px
   - .item2 = 500px - 117.65px ≈ 382.35px
   - .item3 = 400px - 47.06px ≈ 352.94px

验证：264.71 + 382.35 + 352.94 ≈ 1000px ✓
```
